        ...
        for sentence, aspect, opinion in tqdm(zip(sentences, aspects,
            ...
            for idx, token in enumerate(tokens):  # Loop over each word in the sentence
                if token in aspect_words:  # Is the word one of our aspect words? E.g. is the word a noun? Remember aspect words are just all nouns right now
                    cntAspects += 1
                    replacements = self.tokenizer.convert_ids_to_tokens(
                        word_ids[idx])  # Get the top 30 replacements for this noun.
                    for repl in replacements:  # Loop over each replacement
                        ...
                        for cat in categories:  # For category in ['food', 'place', 'service']
                            if repl in aspect_sets[
                                cat]:  # Is the replacement in our generated vocab for this category? Is this the top M words? Or all of them? When do we filter just the top M? I think the authors forget to filter by the top M words.
                                cat_scores[cat] = cat_scores.get(cat,
                                                                 0) + 1  # Increment the 'score' for this category by 1
                                break
            ...
            for cat in categories:  # ['food', 'place', 'service']
                val = cat_scores.get(cat, 0) / max(cntAspects,
                                                   1)  # Of all the nouns in the sentence, we get ~30 replacement words. If one of the replacements is in our vocab, then increment the score for that category. Here we get the score for this category for this sentence, then divide by number of nouns. So essentially this is 'for each noun in the sentence, on average, how many of the replacement words are in our vocab?'
            ...
            f.write(
                summary)  # Example summary string: this place is not pretentious and down to earth . very simple and affordable mexican # # # fare . it 's comparable to los favs . carne # # # asada fries are my favorite . great mexican place within walking # # # distance from asu . \n food: 3.888888888888889 place: 5.111111111111111 service: 0.7777777777777778 negative: 0.0 positive: 3.3125
            f.write('\n')
        f.close()

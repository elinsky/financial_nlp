    ...
    def __call__(self):
        ...
        with open(f'{self.root_path}/scores.txt', 'r') as f:
            for idx, line in enumerate(
                    f):  # Loop over each sentence in the training set. This is the scores file. So line 1 is a sentence. Line 2 are the scores for each category
                if idx % 2 == 1:  # If line is the scores (not the sentence)
                    ...
                    for j, val in enumerate(values):
                        if j % 2 == 1:
                            dist[values[j - 1][:-1]].append(
                                float(val))  # Save the sentence scores to the distribution dict
        ...
        with open(f'{self.root_path}/scores.txt', 'r') as f:
            ...
            for idx, line in enumerate(f):  # Loop over all the lines in the scores file.
                if idx % 2 == 1:  # Only look at lines that are scores, not the actual sentences
                    ...
                    if len(aspect) == 1 and len(
                            sentiment) == 1:  # Only add to label.txt the sentences that clearly have 1 dominant aspect category, and 1 dominant opinion category.
                        nf.write(sentence)
